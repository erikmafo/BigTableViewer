...
public class BigtableViewController {
    ...
    private final ObservableList<BigtableRow> rows = FXCollections.observableArrayList();
    ...
    public BigtableViewController(
        ...
        valueConverter.bind(Bindings.createObjectBinding(() -> {
        ...
        valueConverter.addListener((obs, prev, current) -> tableView.refresh());
        ...
        table.addListener((obs, prev, current) -> rows.clear());
    }

    public ObservableList<BigtableRow> getRows() {
        return rows;
    }
    ...
    @FXML
    private void handleTableSettingsButtonAction(ActionEvent actionEvent) {
        TableSettingsDialog
